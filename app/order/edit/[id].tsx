...
import { Order, OrderWithDetails, PaymentStatus, ClientSource, PAYMENT_STATUS_LABELS, CLIENT_SOURCE_LABELS } from '@/lib/types';
...
export default function EditOrderScreen() {
  ...
  const { users, getOrderWithDetails, updateOrder, deleteOrder, refreshOrders } = useData();
  ...
  const loadOrder = useCallback(async () => {
    if (id) {
      ...
      if (data) {
        ...
        setDeliveryDate(format(new Date(data.deliveryDateTime), 'dd.MM.yyyy'));
        setDeliveryTime(format(new Date(data.deliveryDateTime), 'HH:mm'));
        setAmount(data.amount.toString());
        setComment(data.comment || '');
        setFloristId(data.floristId);
        setCourierId(data.courierId);
        if (data.externalFloristName) {
          setUseExternalFlorist(true);
          setExternalFloristName(data.externalFloristName);
          setExternalFloristPhone(data.externalFloristPhone || '');
        }
        if (data.externalCourierName) {
          setUseExternalCourier(true);
          setExternalCourierName(data.externalCourierName);
          setExternalCourierPhone(data.externalCourierPhone || '');
        }
        if (data.deliveryDateTimeEnd) setDeliveryTimeEnd(format(new Date(data.deliveryDateTimeEnd), 'HH:mm'));
        if (data.paymentStatus) setPaymentStatus(data.paymentStatus as PaymentStatus);
        setPaymentMethod(data.paymentMethod || '');
        setPaymentDetails(data.paymentDetails || '');
        if (data.clientSource) setClientSource(data.clientSource as ClientSource);
        setClientSourceId(data.clientSourceId || '');
      }
      setLoading(false);
    }
  ...
  const handleDelete = () => {
    confirmAction('Удалить заказ?', 'Это действие нельзя отменить', async () => {
      if (order) {
