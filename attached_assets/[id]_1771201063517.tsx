...
function getPhotoUrl(uri: string): string {
  ...
  const handleAddPhoto = async () => {
    ...
    if (!result.canceled && result.assets[0] && order) {
      ...
      try {
        ...
        await addAttachment(order.id, base64Data, mimeType);
        await loadOrder();
      } catch (error: any) {
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              {(isFlorist || isManager || isOwner) && (
                <Pressable
                  style={[styles.addPhotoButton, uploading && { opacity: 0.5 }]}
                ...
                >
                  ...
                  <Text style={styles.addPhotoText}>{uploading ? 'Загрузка...' : 'Добавить'}</Text>
                </Pressable>
              )}
            </View>
              ...
              <View style={styles.photoGrid}>
                {order.attachments.map(attachment => (
                  <Pressable
                    ...
                    onPress={() => setSelectedPhoto(getPhotoUrl(attachment.uri))}
                    onLongPress={() => handleDeleteAttachment(attachment.id)}
                  >
                    <Image source={{ uri: getPhotoUrl(attachment.uri) }} style={styles.photo} />
                  </Pressable>
                ))}
              </View>
            ) : (
